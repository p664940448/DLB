var vm = new Vue({
	...
	data: {
        ...
        forms:{
            ...
            technologyaudit: {
                ...
                form: {
                    ...
                    storeAccessType: '',
                    storeId: '',
                    storePath: '/temp/%(date)/%(file)_%(time)_%(timestamp).jpg',
                    tag:''
                },
                oldform: {}
            },
            fileregister:{
                visible: false,
                form: {
                    createPackageCaseNoPackageInfo: 'Y'
                },
                oldform: {}
            },
            endPoint: {
        ...
        currentNodePk: 0,
        currentNodeId: '',
        fileRegisterTaskType:'fileregister'
	},
	methods: {
		...
		addItem: function(nodeId,nodeType){
			var _self = this;
			if(nodeType === _self.fileRegisterTaskType && _self.hasFileRegisterNode() ){
			    _self.$message("文件注册节点只能有一个！");
			    return;
            }
        ...
        hasFileRegisterNode: function(){
		    //判断是否已经有文件注册节点
            var _self = this;
            for( i in _self.nodes){
                if( _self.nodes[i].taskType == _self.fileRegisterTaskType){
                    return true;
                }
            }
            return false;
	...
	created: function() {
        ...
        getData("getGroupList.do",null,_self,function(response){
            _self.groupList = response;
        });
